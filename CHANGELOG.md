...
* **Post rich-text messages.** Bot presets can post using Telegram's Rich Messages: headings, tables, ordered and task lists, block quotes, collapsible blocks, footnotes, formulas, inline media and more. Pick the "Bot + rich text" method when creating a preset or in the advanced publishing settings later. Two important limitations, *imposed by Telegram*: (1) Rich Messages can accept only web-embedded media (HTTP/HTTPS links), local file attachments are not supported; (2) You cannot edit Rich Message after publication.
