export default {
    ...
    SETTING_MD_EMBEDS_AS_COMMENTS_DESC: "Когда включено, содержимое .md-вложений отправляется как комментарии в обсуждение поста; когда выключено, .md-файлы прикрепляются как документы",
...
Ограничение rich-text сообщений: к ним нельзя прикреплять локальные файлы (веб-встраивания файлов поддерживаются). Это ограничение установлено самим Telegram.`,
    ...
    SETTING_BOT_TOKEN_SHOW_HIDE: "Показать / скрыть токен",
    ...
    NOTICE_ERR_RICH_LOCAL_MEDIA: "Ошибка: Rich-сообщения поддерживают только веб-медиа — локальные вложения нельзя отправить методом «бот + rich».",
    ...
    NOTICE_RICH_COMMENT_UNAVAILABLE: "Rich-комментарий недоступен — отправляется как обычный текст. ({error})",
    NOTICE_COMMENT_DISCUSSION_NOT_FOUND: "Не удалось найти сообщение в обсуждении для комментария.",
    ...
    ERR_NO_CHAT_TARGETS: "Для этого пресета не настроены целевые чаты.",
