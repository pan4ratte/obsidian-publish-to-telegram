export default {
    ...
    SETTING_ADD_CHANNEL_DESC: `Пресет «Пользователь» — публикует от имени вашего личного аккаунта Telegram (требуется единовременная авторизация).
1. Авторизуйтесь в аккаунт и убедитесь, что у вас есть права для постинга в целевые каналы/группы.
2. Создайте пресет «Пользователь» и нажмите на поле поиска для загрузки списка чатов. @username или ID можно ввести вручную.

Пресет «Бот» — публикует через токен Telegram-бота (авторизация аккаунта не нужна).
1. Создайте бота через @BotFather и назначьте его администратором целевого канала/группы.
2. Создайте пресет «Бот», введите токен бота, затем добавьте ID чатов или @username.
...
** В руководстве пользователя детально описаны все функции плагина и поддерживаемые опции форматирования.`,
    ...
    SETTING_ADD_BOT_PRESET: "Новый пресет Бот",
    PRESET_BADGE_USER: "USER",
    PRESET_BADGE_BOT: "BOT",
    SETTING_BOT_TOKEN_NAME: "Токен бота",
    SETTING_BOT_TOKEN_DESC: "Токен от @BotFather. Бот должен быть администратором целевого канала/группы.",
